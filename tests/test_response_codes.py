...
class TestResponseAndBrowser(unittest.TestCase):
    """Regression tests for propagating actual HTTP codes into outputs and reports."""
    ...
    def test_response_handle_returns_response_code_for_success_and_subdomain_fallback(self):
        """Response.handle() should expose actual response code for downstream output consumers."""
        ...
        status, url, size, code = response_handler.handle(
            self.make_response(status=403, body=b'nope', headers={'Content-Length': '4'}),
        ...
        self.assertEqual((status, url, size, code), ('forbidden', 'http://example.com/path', '4B', '403'))
    ...
    def test_browser_catch_report_data_keeps_legacy_items_and_detailed_codes(self):
        """Browser.__catch_report_data() should preserve old buckets and add response code metadata."""
...
class TestDebugOutput(unittest.TestCase):
    """Console output regression tests for actual response codes."""

    def test_debug_request_uri_passes_response_code_into_template(self):
        """Debug.debug_request_uri() should render the actual response code in console output."""

        cfg = Config({'debug': 1, 'reports': 'std'})
        ...
        with patch('src.lib.browser.debug.tpl.info') as info_mock, \
            ...
            debug.debug_request_uri('success', 'http://test.local/data/', items_size=1, total_size=1, content_size='2B', response_code='200')

        self.assertEqual(info_mock.call_args.kwargs['code'], '200')


class TestReporterPlugins(unittest.TestCase):
    """Report plugin tests for actual response code export."""
    ...
    def test_plugin_provider_falls_back_to_legacy_items_when_report_items_are_missing(self):
        """PluginProvider should enrich legacy URL-only buckets with default code and size."""
    ...
    def test_text_report_includes_response_code_and_skips_failed_bucket(self):
        """Text reports should include actual response codes for non-failed buckets only."""
    ...
    def test_json_report_contains_report_items_with_response_codes(self):
        """JSON reports should expose detailed report_items with actual response codes."""
        ...
        self.assertIn('"code": "200"', content)
